...
# Adapted from demo_rfcn.py by Yuwen Xiong
# --------------------------------------------------------

"""
Demo script which crops the detection areas, scales them and
convert each of them to its Super Resolution (SR) counter part.
...
def save_detections(im, class_name, dets, path, thresh=0.5):
    ...
    for i in inds:
        ...
        score = dets[i, -1]
        print bbox
        patch = im[bbox[0]:bbox[2], bbox[1]:bbox[3]]
...
def get_detections(net, image_name):
    ...
    # Path to store crops
    crop_dest_location = os.path.join(cfg.DATA_DIR, 'demo')
